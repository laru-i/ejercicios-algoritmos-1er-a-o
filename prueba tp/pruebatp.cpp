...
void mostrarArchivo(Clase matClase[niveles][idiomas]);
...
void mostrarArchivo(Clase matClase[niveles][idiomas])
{
    ...
    while (string(nomArch) != "f")
    {
        ...
        {
            Clase exa;
            fread(&exa, sizeof(Clase), 1, ae);
            while (!feof(ae))
            {
                cout << exa.codigo << " " << exa.nivel << " " << exa.cupos << " " << exa.dniProfe << " " << exa.nombre << endl;
                fread(&exa, sizeof(Clase), 1, ae);
            }
            fclose(ae);
        }
